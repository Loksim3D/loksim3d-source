...
{
	std::wstring outStr;
	UINT ret = lhWinAPI::GetTempFileName(path, L"l3d", 0, outStr);
	if (ret != ERROR_BUFFER_OVERFLOW && ret != 0)
	{
		KTMTransaction txTest;
		HANDLE hFile = txTest.CreateFile(outStr.c_str(), GENERIC_WRITE, 0,
		...
		{
			m_TxFsDisabled = false;
			CloseHandle(hFile);
			txTest.Commit();
		}
		::DeleteFile(outStr.c_str());
	}
	else
	{
		m_TxFsDisabled = true;
	}
	return !m_TxFsDisabled;
}
...
{
	if(useTransactionsIfAvailable && !m_TxFsDisabled)
	{
...
bool KTMTransaction::UseTransactedFunctions() const
{
...
BOOL KTMTransaction::CreateDirectoryEx(LPCTSTR lpTemplateDirectory, LPCTSTR lpNewDirectory, LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
	...
	}else if (lpTemplateDirectory == nullptr) {
		return ::CreateDirectory(lpNewDirectory, lpSecurityAttributes);
	} else {
